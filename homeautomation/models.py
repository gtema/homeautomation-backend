...
class StockProduct(db.Model):
    ...
    id = db.Column(db.Integer, primary_key = True, autoincrement = True)
    category_id = db.Column(db.Integer, db.ForeignKey('cat_category.id'),  nullable = False)
    name = db.Column(db.String(255), nullable = False)
    ...
    @property
    def amount(self):
        items = StockProductItem.query.filter(StockProductItem.product_id == self.id,  StockProductItem.amount > 0)
    ...
    def first_started_id(self):
        item = StockProductItem.query.filter(StockProductItem.product_id == self.id,  StockProductItem.is_started == True).order_by(StockProductItem.expiry_date).first()
        return item.id
...
class StockProductItem(db.Model):
    ...
    id = db.Column(db.Integer,  primary_key = True,  autoincrement = True)
    product_id = db.Column(db.Integer,  db.ForeignKey('cat_product.id'),  nullable = False)
